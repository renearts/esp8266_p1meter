...
void send_metric(String name, float metric)
{
    ...
    char output[10];
//    ltoa(metric, output, sizeof(output)); // I'd rather have float outputs in kWh
    dtostrf(metric,1,3,output);
...
{  
//    send_metric("timestamp", TIMESTAMP); // TIMESTAMP is not set to value
    ...
    if (strncpy(res, buffer + s + 1, l))
    {
        if (endchar == '*')
        {
            if (isNumber(res, l))
                ...
                return (atof(res)); // keep kWh format
        }
        else if (endchar == ')')
        {
            if (isNumber(res, l))
...
bool decode_telegram(int len)
{
    ...
    // 0-0:1.0.0 = Timestamp: "2010-12-09 11:30:20 Wintertime" (DSMR v4.0) 
//    if (strncmp(telegram, "0-0:1.0.0", strlen("0-0:1.0.0")) == 0)
//    {
//        TIMESTAMP = getValue(telegram, &TIMESTAMP, len, '(', ')');  // Will return 0 as timestamp is a string
//    }
